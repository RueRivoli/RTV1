...
/*   Updated: 2017/10/13 19:33:21 by fgallois         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_hit_point			nearest_point_after_object(t_env *env, t_ray \
		ray, t_hit_point mem, t_obj *obj_met);
float				distance_with_next_intersection(t_env *env, t_vect v, \
		t_obj **colore);
int					is_light_reached(t_light *light, t_env *env, t_hit_point \
		mem, t_obj *colore);
void				put_on_light(t_env *env, t_hit_point mem, t_obj *colore);
void				raytrace_thread(t_env *env, int pi, int pf);
t_hit_point			compare_distance(t_hit_point mem, t_hit_point hp, float \
		*minimum, t_hit_point nearest_hp);
...
t_hit_point			new_hit_point(t_vect vect, float dist_to_cam, t_vect \
		normal, int form);
